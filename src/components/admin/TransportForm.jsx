...
const TransportForm = ({ onSuccess, onCancel, initialData = null }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.transport_name.trim()) return toast({ title: "Transport Name is required", variant: "destructive" });
    ...
    try {
      ...
      let result;
      if (initialData?.id) {
          const { data, error } = await supabase.from('transports').update(payload).eq('id', initialData.id).select().single();
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="transport_name" name="transport_name" value={formData.transport_name} onChange={handleChange} required />
        </div>
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="space-y-2 relative">
                ...
                <div className="relative">
                    <Input id="pincode" name="pincode" value={formData.pincode} onChange={handlePincodeChange} maxLength={6} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="vehicle_type" name="vehicle_type" value={formData.vehicle_type} onChange={handleChange} placeholder="e.g. Truck, Van" />
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button type="submit" disabled={isSubmitting}>
          ...
          {initialData ? 'Update Transport' : 'Create Transport'}
        </Button>
      </div>
    </form>
  );
