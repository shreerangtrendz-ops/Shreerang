...
function buildDimensionRows(bills, dimFn, label) {
  const map = {};
  bills.forEach(b => {
...
// ─── Party-wise table (detailed, with individual bills on expand) ─
function PartyTable({ bills }) {
  const [expanded, setExpanded] = useState(null);

  const partyRows = buildDimensionRows(bills, b => b.customer_name, 'Party');
...
export default function OutstandingReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Fetch sales_bills (only unpaid, total_amount > 0)
    ...
    // Fetch all receipt payment lines (type=Receipt)
  ...
  const partyRows   = buildDimensionRows(filtered, b => b.customer_name);
