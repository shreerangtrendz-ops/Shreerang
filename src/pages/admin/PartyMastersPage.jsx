...
function InfoRow({ label, value }) {
  ...
  return (
    <div style={{display:'flex',gap:8,fontSize:12,padding:'5px 0',borderBottom:`1px solid ${T.bg}`}}>
      <span style={{color:T.muted,minWidth:120,flexShrink:0,fontWeight:500}}>{label}</span>
...
function CustomerProfile({ customer, onClose }) {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages:[{
            role:'user',
            content:`You are a textile business analyst for SheeRang Trendz Pvt. Ltd. (Surat). Analyse this customer and give a SHORT 3-4 line business summary covering: buying pattern, loyalty, payment behaviour, and one actionable recommendation.\n\nCustomer: ${customer.name}\nCity: ${customer.city}, ${customer.state}\nAgent: ${customer.agent_name || 'Direct'}\nType: ${customer.customer_type}\nCredit Days: ${customer.credit_days || 0}\nTotal Sales: ${Math.round((stats?.total||0)/100000)}L across ${stats?.bills} bills\nLast Order: ${fmtD(stats?.lastDate)}\nTotal Metres: ${Math.round(stats?.mtrs||0)}m\nDesigns: ${stats?.designs?.slice(0,8).join(', ')}\n\nBe concise. Plain paragraph only.`
          }]
        })
      });
  ...
  return (
    <div style={{marginTop:20,background:T.surface,border:`2px solid ${T.teal}`,borderRadius:12,overflow:'hidden'}}>
      <div style={{background:`linear-gradient(135deg, ${T.navy} 0%, #1a4a45 100%)`,padding:'18px 22px',color:'#fff'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              {customer.customer_type && <span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:10,background:'rgba(255,255,255,0.15)',color:'#fff'}}>{customer.customer_type}</span>}
              {customer.status && <span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:10,background:customer.status==='active'?'#065F46':'#7F1D1D',color:'#fff'}}>{customer.status}</span>}
            </div>
          </div>
          <button onClick={onClose} style={{background:'rgba(255,255,255,0.15)',border:'none',color:'#fff',width:28,height:28,borderRadius:6,cursor:'pointer',fontSize:16,flexShrink:0}}>\u2715</button>
        </div>
      </div>
      <div style={{padding:'20px 22px'}}>
        {stats && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:20}}>
            ...
            <StatCard label="Designs" value={stats.designs.length} sub="unique" color={T.green}/>
          </div>
        )}
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Contact & Location</div>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Business Info</div>
        ...
        {bills.length > 0 && (
          <div style={{marginBottom:20}}>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Recent Bills</div>
                ...
                <thead>
                  <tr style={{background:T.tealLight}}>
                    {['Bill No','Date','Metres','Amount','Design','Status'].map(h => (
                      <th key={h} style={{padding:'6px 10px',textAlign:'left',fontSize:10,fontWeight:700,color:T.navy,textTransform:'uppercase',borderBottom:`1px solid ${T.border}`}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {bills.map((b,i) => (
                    <tr key={b.bill_number||i} style={{borderBottom:`1px solid ${T.bg}`,background:i%2===0?T.surface:'#FAFCFC'}}>
                      ...
                      <td style={{padding:'6px 10px'}}><span style={{fontSize:10,fontWeight:700,padding:'1px 6px',borderRadius:8,background:b.status==='paid'?'#D1FAE5':'#FEF3C7',color:b.status==='paid'?'#065F46':'#92400E'}}>{b.status||'\u2014'}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={{background:'#F0F9FF',border:'1px solid #BAE6FD',borderRadius:8,padding:'14px 16px',marginBottom:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <div style={{fontSize:11,fontWeight:800,color:'#0C4A6E',textTransform:'uppercase',letterSpacing:'0.08em'}}>AI Business Summary</div>
            {!aiSummary && (
              <button onClick={getAI} disabled={aiLoading||!stats}
                style={{padding:'5px 12px',background:T.teal,color:'#fff',border:'none',borderRadius:6,fontSize:11,fontWeight:700,cursor:aiLoading?'wait':'pointer',opacity:(!stats||aiLoading)?0.6:1}}>
          ...
          {aiSummary
            ...
            : <p style={{margin:0,fontSize:11,color:'#64748B',fontStyle:'italic'}}>Click "Generate Analysis" for AI-powered business summary.</p>
          }
        </div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <a href={`/admin/reports/party-ledger?party=${encodeURIComponent(customer.tally_ledger_name||customer.name)}`}
            style={{padding:'8px 16px',background:T.teal,color:'#fff',borderRadius:7,fontSize:12,fontWeight:700,textDecoration:'none'}}>Full Ledger</a>
          <a href={`/admin/smart-outstanding?search=${encodeURIComponent(customer.name)}`}
            style={{padding:'8px 16px',background:'#EEF8F6',color:T.navy,border:`1px solid ${T.border}`,borderRadius:7,fontSize:12,fontWeight:700,textDecoration:'none'}}>Outstanding</a>
          <a href={`/admin/accounting/sales-bills?customer=${encodeURIComponent(customer.name)}`}
            style={{padding:'8px 16px',background:'#EEF8F6',color:T.navy,border:`1px solid ${T.border}`,borderRadius:7,fontSize:12,fontWeight:700,textDecoration:'none'}}>All Bills</a>
        </div>
      </div>
    </div>
  );
...
function AgentProfile({ agent, custCounts, onClose }) {
  ...
  useEffect(() => {
    ...
    supabase.from('sales_bills').select('total_amount,bill_date,comm_amount,commission_amount').eq('agent_name', name)
      .then(({ data }) => {
        const rows = data || [];
        const total = rows.reduce((s,r)=>s+Number(r.total_amount||0),0);
    ...
    <div style={{marginTop:20,background:T.surface,border:`2px solid ${T.teal}`,borderRadius:12,overflow:'hidden'}}>
      ...
      <div style={{padding:'20px 22px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Contact</div>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Commission Info</div>
...
function SupplierProfile({ supplier, onClose }) {
  ...
  useEffect(() => {
    if (!supplier) return;
    supabase.from('grey_purchase').select('total_amount,voucher_date,qty_mtrs,purchase_rate')
      .eq('supplier_name', supplier.supplier_name).order('voucher_date',{ascending:false}).limit(100)
      .then(({ data }) => {
        const rows = data || [];
        const total = rows.reduce((s,r)=>s+Number(r.total_amount||0),0);
  ...
  return (
    <div style={{marginTop:20,background:T.surface,border:`2px solid ${T.teal}`,borderRadius:12,overflow:'hidden'}}>
      ...
      <div style={{padding:'20px 22px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Business Details</div>
            ...
            <InfoRow label="Last Rate" value={supplier.last_purchase_rate>0?fmt(supplier.last_purchase_rate)+'/m':null}/>
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:T.muted,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Bank Details</div>
  ...
  return (
    <div>
      <div style={{display:'flex',gap:10,marginBottom:12,flexWrap:'wrap'}}>
        <input value={search} onChange={e=>{setSearch(e.target.value);setPage(0);}} placeholder="Search customer name\u2026"
          style={{flex:1,minWidth:180,padding:'8px 12px',border:`1px solid ${T.border}`,borderRadius:8,fontSize:13,outline:'none',background:T.surface}}/>
      ...
      <div style={{background:'#EEF8F6',border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 14px',marginBottom:12,fontSize:12,color:T.navy,display:'flex',alignItems:'center',gap:8}}>
        <span>\uD83D\uDDFA\uFE0F</span>
      ...
      <div style={{display:'flex',gap:8,justifyContent:'center',marginTop:14,alignItems:'center'}}>
        <button onClick={()=>setPage(p=>Math.max(0,p-1))} disabled={page===0}
          style={{padding:'5px 14px',borderRadius:6,border:`1px solid ${T.border}`,background:T.surface,cursor:page===0?'not-allowed':'pointer',fontSize:12,opacity:page===0?0.4:1}}>\u2190 Prev</button>
        ...
        <button onClick={()=>setPage(p=>p+1)} disabled={rows.length<PAGE}
          style={{padding:'5px 14px',borderRadius:6,border:`1px solid ${T.border}`,background:T.surface,cursor:rows.length<PAGE?'not-allowed':'pointer',fontSize:12,opacity:rows.length<PAGE?0.4:1}}>Next \u2192</button>
      </div>
...
function AgentsTab() {
  ...
  const filtered = rows.filter(r => !search || r.name?.toLowerCase().includes(search.toLowerCase()) || r.agent_name?.toLowerCase().includes(search.toLowerCase()));
  ...
  const columns = [
    ...
    { key:'commission_percentage', label:'Commission %', render: r => (
      <span style={{fontWeight:700,color:T.green}}>{r.commission_percentage?`${parseFloat(r.commission_percentage).toFixed(2)}%`:'\u2014'}</span>
    )},
    { key:'customers', label:'Customers', render: r => <span style={{fontWeight:700,color:T.blue,fontSize:14}}>{custCounts[r.agent_name||r.name]||0}</span> },
    { key:'phone', label:'Phone' },
    { key:'status', label:'Status', render: r => (
      <span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:10,background:r.status==='active'?'#D1FAE5':'#FEF2F2',color:r.status==='active'?'#065F46':'#991B1B'}}>{r.status||'\u2014'}</span>
    )},
  ];
  ...
  return (
    <div>
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search agent name\u2026"
        style={{width:'100%',padding:'8px 12px',border:`1px solid ${T.border}`,borderRadius:8,fontSize:13,outline:'none',background:T.surface,marginBottom:12,boxSizing:'border-box'}}/>
...
function SuppliersTab() {
  ...
  const columns = [
    ...
    { key:'credit_days', label:'Credit', render: r => r.credit_days>0?`${r.credit_days}d`:'\u2014' },
    { key:'bank', label:'Bank', render: r => r.bank_name ? <span style={{fontSize:11,fontWeight:700,color:T.green}}>\u2713 {r.bank_name}</span> : <span style={{fontSize:11,color:T.muted}}>\u2014</span> },
    { key:'status', label:'Status', render: r => (
      <span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:10,background:r.status==='active'?'#D1FAE5':'#FEF2F2',color:r.status==='active'?'#065F46':'#991B1B'}}>{r.status||'\u2014'}</span>
    )},
  ];
...
function TransportersTab() {
  ...
  const filtered = rows.filter(r => !search || r.name?.toLowerCase().includes(search.toLowerCase()) || r.city?.toLowerCase().includes(search.toLowerCase()));
      ...
      {selected && (
        <div style={{marginTop:16,background:T.surface,border:`2px solid ${T.teal}`,borderRadius:12,padding:20}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
            ...
            <button onClick={()=>setSelected(null)} style={{border:'none',background:'none',cursor:'pointer',fontSize:18,color:T.muted}}>\u2715</button>
          </div>
    ...
    <div style={{padding:'24px 28px',background:T.bg,minHeight:'100vh',fontFamily:'system-ui,sans-serif'}}>
      <div style={{marginBottom:24,display:'flex',alignItems:'center',gap:12}}>
        ...
        <div>
          ...
          <p style={{margin:0,fontSize:13,color:T.muted}}>Customers \u00B7 Agents \u00B7 Suppliers \u00B7 Transporters \u2014 click any row to view full profile</p>
        </div>
      </div>
