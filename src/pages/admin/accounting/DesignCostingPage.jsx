              ...
              {filtered.map((r,i)=>{
                ...
                return (
                  <>
                    ...
                    {expanded===i && (
                      <tr><td colSpan={14} style={{padding:'0 10px 14px',background:'#FAFFFE'}}>
                        <div style={{paddingTop:10}}>
                          ...
                          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8}}>
                            {[
                              ['Lot No',           r.lot_no||'—',               T.text, false],
                              ['Supplier',         r.supplier_name||'—',        T.text, false],
                              ['Supplier Invoice', r.supplier_invoice_no||'—',  T.blue, true ],
                              ['Process Date',     fmtD(r.process_date),        T.muted, false],
                              ['Grey Issued',      fmtQ(r.grey_issued_qty_mtrs),T.text, false],
                              ['Grey Received',    fmtQ(r.finish_qty_mtrs),     T.green, false],
                              ['Shortage',         fmtQ(r.short_qty_mtrs)+' ('+Number(r.shortage_pct||0).toFixed(2)+'%)', T.red, false],
                              ...
                              ['Broker Commission',fmt(r.broker_commission||0), T.orange, false],
                              ['Gross Margin %',   pct(r.gross_margin_pct),     Number(r.gross_margin_pct||0)>=0?T.green:T.red, true ],
                              ['Sales Bills',      String(r.sales_bills_count||0), T.muted, false],
                              ['Sold Qty',         fmtQ(r.sold_qty_mtrs||0),    T.green, false],
                              ['Unsold Qty',       fmtQ(r.unsold_qty_mtrs||0),  T.gold, false],
                              ['Tally Stock',      r.current_stock_mtrs!=null ? fmtQ(r.current_stock_mtrs) : '—', T.teal, false],
                            ].map(([lbl,val,col,bold])=>(
                              <div key={lbl} style={{background:T.surface,borderRadius:6,padding:'6px 10px',border:`1px solid ${bold?col:T.border}`,borderLeft:bold?`3px solid ${col}`:`1px solid ${T.border}`}}>
                                <div style={{fontSize:8,color:T.muted,textTransform:'uppercase',letterSpacing:'.4px'}}>{lbl}</div>
                                <div style={{fontSize:11,fontWeight:bold?700:500,color:col,marginTop:1}}>{val}</div>
                              </div>
                            ))}
                          </div>
                        </div>
