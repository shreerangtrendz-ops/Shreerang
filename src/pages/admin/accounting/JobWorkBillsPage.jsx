...
export default function JobWorkBillsPage() {
  ...
  async function fetchBills() {
    ...
    const [manual, synced] = await Promise.all([
      ...
      supabase.from('process_issues').select('*').order('issue_date', { ascending:false }).limit(1000)
    ]);
    const combined = [
        ...
        <div style={{ ...CARD, padding:0, overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            ...
            <tbody>
              ...
              :filtered.length===0?<tr><td colSpan={10} style={{ padding:30, textAlign:'center', color:'#6A9B95' }}>No bills yet. Add one above.</td></tr>
              :filtered.map(b=>(
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20 }}>
          <div style={{ background:'#fff', borderRadius:14, padding:24, width:'100%', maxWidth:800, maxHeight:'90vh', overflowY:'auto' }}>
            ...
            <div style={{ background:'#F8FAFC', padding:16, borderRadius:8, marginBottom:16, border:'1px solid #E2E8F0' }}>
              <div style={{ fontSize:12, fontWeight:700, color:'#475569', marginBottom:12, textTransform:'uppercase' }}>1. Bill & Contractor Details</div>
            ...
            <div style={{ background:'#F8FAFC', padding:16, borderRadius:8, marginBottom:16, border:'1px solid #E2E8F0' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#475569', textTransform:'uppercase' }}>2. Fabric Issued (Line Items)</div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              <div style={{ background:'#F0FDF4', padding:16, borderRadius:8, border:'1px solid #BBF7D0' }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#166534', marginBottom:12, textTransform:'uppercase' }}>3. Taxes & Total</div>
              ...
              <div style={{ background:'#F8FAFC', padding:16, borderRadius:8, border:'1px solid #E2E8F0', display:'flex', flexDirection:'column', gap:12 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#475569', textTransform:'uppercase' }}>4. Notes</div>
                <textarea placeholder="Remarks, challan reference..." value={form.notes||''} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} style={{ flex:1, padding:'8px 10px', borderRadius:7, border:'1px solid #CBD5E1', fontSize:13, boxSizing:'border-box', resize:'none' }} />
              </div>
            </div>
            <div style={{ display:'flex', gap:10, marginTop:20 }}>
              <button onClick={saveBill} disabled={saving} style={BTN({ background:'linear-gradient(135deg,#3DBFAE,#2BA898)', color:'#fff', flex:1, padding:'12px', fontSize:14 })}>
                {saving?'Posting Bill\u2026':'Save Job Work Bill'}
              </button>
