...
const CostingCalculatorPage = () => {
    ...
    useEffect(() => {
        ...
        // Auto-fetch latest Tally rates
        async function fetchTallyRates() {
            try {
                setTallyRatesLoading(true);
                // Fetch grey fabric rate
                ...
                const { data: processData } = await supabase
                    ...
                    .select('process_type, shortage_pct')
                    .order('date', { ascending: false })
                    .limit(50);

                let latestShortage = '';
                if (processData) {
                    const dyeObj = processData.find(p => p.process_type?.toLowerCase().includes('dye'));
                    if (dyeObj && dyeObj.shortage_pct) latestShortage = dyeObj.shortage_pct;
                }

                setFormData(prev => ({
                    ...prev,
                    rate: greyData?.[0]?.price?.toString() || prev.rate,
                    process_shortages: latestShortage?.toString() || prev.process_shortages
                }));
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto pb-20">
            ...
            <Card className="border-slate-200 shadow-sm">
                <CardHeader>
                    <CardTitle>
                        Step {step} of 3: {step === 1 ? 'Select Path' : step === 2 ? 'Enter Costs' : 'Review & Save'}
                    </CardTitle>
                    <CardDescription className="text-sm">
                        {step === 2 && tallyRatesLoading ? 'Loading live Tally rates...' : step === 2 && !tallyRatesLoading ? 'Live rates prefilled from Tally Data Sync.' : ''}
                    </CardDescription>
