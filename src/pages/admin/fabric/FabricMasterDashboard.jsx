...
import { 
  Plus, UploadCloud, Trash2, Edit, Layers, Palette, Sparkles, Filter 
} from 'lucide-react';
...
const FabricMasterDashboard = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [baseRes, finishRes, fancyBaseRes, fancyFinishRes] = await Promise.all([
        ...
        supabase.from('finish_fabrics').select('*, base_fabrics(base_fabric_name)').order('created_at', { ascending: false }),
        supabase.from('fancy_base_fabrics').select('*').order('created_at', { ascending: false }),
        supabase.from('fancy_finish_fabrics').select('*, finish_fabrics(finish_fabric_name)').order('created_at', { ascending: false })
      ]);

      if (baseRes.error) throw baseRes.error;
      if (finishRes.error) throw finishRes.error;
      // Accept errors for tables that might not exist yet during migration
  ...
  const filterData = (items, type) => {
    ...
    return items.filter(item => {
      ...
      if (filters.search) {
        ...
        const nameMatch = (item.name || item.fabric_name || item.base_fabric_name || item.finish_fabric_name || '').toLowerCase().includes(query);
  ...
  const stats = useMemo(() => {
    ...
    return {
      totalBase: data.base?.length || 0,
      totalFinish: data.finish?.length || 0,
      totalFancyBase: data.fancyBase?.length || 0,
      totalFancyFinish: data.fancyFinish?.length || 0,
          ...
          <TableBody>
            {items.slice(0, ITEMS_PER_PAGE).map((item) => (
              <TableRow key={item.id} className="hover:bg-slate-50">
                ...
                <TableCell>{item.name || item.fabric_name || item.finish_fabric_name || item.base_fabric_name}</TableCell>
                <TableCell>
                  <div className="flex flex-col">
                    <span className="text-sm font-medium">{item.base || item.base_fabrics?.base || '-'}</span>
                    <span className="text-xs text-muted-foreground">{item.process || item.finish_type || '-'}</span>
                  </div>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/fabric-master/${type === 'base' ? '' : type + '/'}${item.id}/edit`)}>
      ...
      {loading ? (
        ...
        <Accordion type="multiple" defaultValue={['base_fabric']} className="space-y-4">
          ...
          <AccordionItem value="base_fabric" className="bg-white border rounded-lg px-4">
            ...
            <AccordionContent className="pt-2 pb-6">
              <div className="flex justify-end gap-2 mb-4">
                <Button variant="outline" size="sm" onClick={() => navigate('/admin/fabric/bulk-import')}>
          ...
          <AccordionItem value="finish_fabric" className="bg-white border rounded-lg px-4">
             ...
             <AccordionContent className="pt-2 pb-6">
                <div className="flex justify-end gap-2 mb-4">
                  ...
                  <Button size="sm" onClick={() => navigate('/admin/fabric/finish/new')}>
                    <Plus className="h-4 w-4 mr-2" /> Add New
                  </Button>
                </div>
          ...
          <AccordionItem value="fancy_base_fabric" className="bg-white border rounded-lg px-4">
             ...
             <AccordionContent className="pt-2 pb-6">
                <div className="flex justify-end gap-2 mb-4">
                  <Button size="sm" onClick={() => navigate('/admin/fabric/fancy-base/new')}>
                    <Plus className="h-4 w-4 mr-2" /> Add New
                  </Button>
                </div>
          ...
          <AccordionItem value="fancy_finish_fabric" className="bg-white border rounded-lg px-4">
             ...
             <AccordionContent className="pt-2 pb-6">
                <div className="flex justify-end gap-2 mb-4">
                  <Button size="sm" onClick={() => navigate('/admin/fabric/fancy-finish/new')}>
                     <Plus className="h-4 w-4 mr-2" /> Add New
                  </Button>
                </div>
