// ============================================================
// TallySyncService.js — v3 FIXED
// Fixes: amount /2 bug, upsert conflicts, proper error handling
...
async function postToTally(xml, company = '') {
  ...
  if (error) throw new Error('Tally edge function error: ' + (error.message || String(error)));
  ...
  if (data.success === false) {
    ...
    throw new Error('Tally error: ' + (data.error || 'Unknown error from Tally'));
  }
...
// ─── VOUCHER PARSER — FIX: removed /2 bug ────────────────────
function parseVouchers(xml, type) {
  ...
  for (const b of blocks) {
    ...
    // FIX: Use the AMOUNT tag directly from the voucher header (not halved sum)
    // Tally returns signed amounts; take the absolute of the positive (credit/debit) side
    const amounts = extractAll(b, 'AMOUNT').map(parseAmt);
    // Filter positives for purchase (supplier credits us), negatives for sales
    const positives = amounts.filter(a => a > 0);
    const negatives = amounts.filter(a => a < 0).map(Math.abs);
    // Use the dominant side (max of either) — correct for both purchase and sales
    const total = positives.length > 0
      ? Math.max(...positives)
      : (negatives.length > 0 ? Math.max(...negatives) : 0);

    const narr = extractTag(b, 'NARRATION') || null;
    if (type === 'Purchase') {
      rows.push({
        bill_number: vn, bill_date: dt, supplier_name: pa,
        total_amount: total, notes: narr, status: 'synced',
        tally_sync_status: 'synced', tally_synced_at: new Date().toISOString()
      });
    } else {
      rows.push({
        bill_number: vn, bill_date: dt, customer_name: pa,
        total_amount: total, notes: narr, status: 'synced',
        tally_sync_status: 'synced', tally_synced_at: new Date().toISOString()
      });
    }
...
export async function pullBillsChunk(billType = 'purchase', company = '') {
  ...
  try {
    ...
    // FIX: default to current fiscal year start (April 1)
    const currentYear = new Date().getFullYear();
    const fiscalStart = (new Date().getMonth() >= 3)
      ? `${currentYear}-04-01`
      : `${currentYear - 1}-04-01`;
    const fromDate = lastDate ? addDays(lastDate, 1) : fiscalStart;

    if (fromDate > today) {
      return {
        success: true, hasMore: false, chunkFrom: null, chunkTo: null,
        recordsThisChunk: 0, totalSynced: state?.total_records_synced || 0,
    ...
    return {
      ...
      chunkFrom: fromDate, chunkTo: toDate,
      recordsThisChunk: rows.length, totalSynced: newTotal,
      nextFrom: hasMore ? addDays(toDate, 1) : null,
      message: hasMore
        ? `Synced ${fromDate} → ${toDate}: ${rows.length} records`
        : `✅ Complete! ${newTotal} records up to ${toDate}`
    };
  ...
  try {
    ...
    for (const item of stockItems) {
      const batches = item.batches && item.batches.length > 0
        ? item.batches
        : [{ name: 'MAIN', closingQty: item.closingQty || 0 }];
      for (const batch of batches) {
        ...
        const { error } = await supabase.from('fabric_stock_live').upsert({
          fabric_sku: item.name, fabric_name: item.name, design_no: designNo,
          closing_qty_mtrs: batch.closingQty || 0, sync_date: today,
          last_tally_sync: new Date().toISOString(),
        }, { onConflict: 'fabric_sku,design_no,sync_date' });
        if (!error) count++;
      }
    }
    await logSync('stock_sync', 'success', count, null, null);
    return { success: true, count };
  } catch (err) {
    await logSync('stock_sync', 'error', 0, null, err.message).catch(() => {});
    ...
    for (const ledger of ledgers) {
      if (table === 'sales_team') {
        await supabase.from(table).upsert(
          { name: ledger.name, phone: ledger.phone || null, is_active: true },
          { onConflict: 'name' }
        );
      } else {
        await supabase.from(table).upsert({
          name: ledger.name, tally_ledger_name: ledger.name,
          phone: ledger.phone || null, address: ledger.address || null,
          gst_number: ledger.gstin || null, status: 'active', ...extra
...
export async function pushOrderToTally(orderId) {
  ...
  try {
    ...
    // FIX: check for Tally error in response
    if (responseXml.includes('<LINEERROR>') || responseXml.includes('Error')) {
      const errMatch = responseXml.match(/<LINEERROR>([^<]+)<\/LINEERROR>/i);
      throw new Error('Tally rejected voucher: ' + (errMatch ? errMatch[1] : 'Unknown error'));
    }
    const voucherNo = extractTag(responseXml, 'VOUCHERNUMBER') || extractTag(responseXml, 'VCHNO');
    await supabase.from('sales_orders').update({
      tally_voucher_no: voucherNo, tally_sync_status: 'synced',
      tally_synced_at: new Date().toISOString()
    }).eq('id', orderId);
    return { success: true, voucherNo };
  } catch (err) {
    await supabase.from('sales_orders').update({
      tally_sync_status: 'failed', tally_error_msg: err.message
    }).eq('id', orderId);
    return { success: false, error: err.message };
  }
}
...
export function buildSalesVoucherXML(order) {
  ...
  const taxableAmt = (amount / (1 + gstRate / 100)).toFixed(2);
  const gstAmt = (amount - parseFloat(taxableAmt)).toFixed(2);
  const isIntraState = true; // adjust based on customer state
  return '<ENVELOPE><HEADER><TALLYREQUEST>Import Data</TALLYREQUEST></HEADER>' +
    ...
    '<ALLLEDGERENTRIES.LIST><LEDGERNAME>' + ledger + '</LEDGERNAME><AMOUNT>' + amount + '</AMOUNT></ALLLEDGERENTRIES.LIST>' +
    (isIntraState
      ? '<ALLLEDGERENTRIES.LIST><LEDGERNAME>CGST @' + (gstRate/2) + '%</LEDGERNAME><AMOUNT>-' + (gstAmt/2).toFixed(2) + '</AMOUNT></ALLLEDGERENTRIES.LIST>' +
        '<ALLLEDGERENTRIES.LIST><LEDGERNAME>SGST @' + (gstRate/2) + '%</LEDGERNAME><AMOUNT>-' + (gstAmt/2).toFixed(2) + '</AMOUNT></ALLLEDGERENTRIES.LIST>'
      : '<ALLLEDGERENTRIES.LIST><LEDGERNAME>IGST @' + gstRate + '%</LEDGERNAME><AMOUNT>-' + gstAmt + '</AMOUNT></ALLLEDGERENTRIES.LIST>'
    ) +
    '<ALLLEDGERENTRIES.LIST><LEDGERNAME>Sales</LEDGERNAME><AMOUNT>-' + taxableAmt + '</AMOUNT></ALLLEDGERENTRIES.LIST>' +
    '</VOUCHER></TALLYMESSAGE></REQUESTDATA></IMPORTDATA></BODY></ENVELOPE>';
}

// ─── PARSE HELPERS ──────────────────────────────────────────
export function parseTallyXMLResponse(xml) {
  const vouchers = [], stockItems = [];
...
function parseLedgersFromXML(xml) {
  const ledgers = [];
  // FIX: Tally returns ledger name as child element <NAME>, not attribute
  for (const m of xml.matchAll(/<LEDGER[^>]*>([\s\S]*?)<\/LEDGER>/gi)) {
    ...
    ledgers.push({
      name,
      gstin: extractTag(b, 'PARTYGSTIN') || extractTag(b, 'GSTIN'),
      phone: extractTag(b, 'LEDGERPHONE') || extractTag(b, 'PHONE'),
      address: extractTag(b, 'ADDRESS'),
      email: extractTag(b, 'EMAIL')
    });
  }
  return ledgers;
}
...
function parseBillsFromXML(xml) {
  ...
  for (const m of xml.matchAll(/<BILLDETAILS[^>]*>([\s\S]*?)<\/BILLDETAILS>/gi)) {
    const b = m[1];
    bills.push({
      partyName: extractTag(b, 'PARTYLEDGERNAME'),
      amount: parseAmt(extractTag(b, 'CLOSINGBALANCE')),
      dueDate: tallyDate(extractTag(b, 'BILLDATE'))
    });
  }
  return bills;
}
...
// Compat exports
export { pullBillsChunk as pullPurchasesFromTally, pullBillsChunk as pullSalesFromTally };
export async function pullJobBillsFromTally() { return { success: false, error: 'Job Bills sync coming in next update' }; }
// v3.0 — Fixed
