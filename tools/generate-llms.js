...
function findReactFiles(dir) {
    return fs.readdirSync(dir)
      ...
      .filter(itemPath => {
              try {
                        return fs.statSync(itemPath).isFile() && /\.(jsx|tsx|js|ts)$/.test(itemPath);
              } catch (e) {
                        return false;
              }
      });
...
function extractHelmetData(content, filePath, routes) {
    ...
    const url = routes.length && routes.has(fileName) ? routes.get(fileName) : generateFallbackUrl(fileName);
    return {
          url,
...
function main() {
  ...
  // If no pages with Helmet found, create a minimal llms.txt to avoid build failure
  if (pages.length === 0) {
        console.log('No pages with Helmet components found - creating minimal llms.txt');
        pages = [{ url: '/', title: 'Shreerang Trendz', description: 'Premium fabric wholesale from Surat, India.' }];
  }
